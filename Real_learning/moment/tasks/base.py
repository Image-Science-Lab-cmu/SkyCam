...
from moment.data.SkyCam_dataloader import DatasetFromFolder
...
class Tasks(nn.Module):
    def __init__(self, args, **kwargs):
        ...
        # dataloader with ours
        self.train_dataloader = self._get_dataloader_SkyCam(data_split="train")
        self.test_dataloader = self._get_dataloader_SkyCam(data_split="test")
        self.val_dataloader = self._get_dataloader_SkyCam(data_split="test")
    ...
    def _get_dataloader_SkyCam(self, data_split: str = "train"):
        print(f"Loading {data_split} split of the dataset")
        if data_split == "train":
            train_loader = DataLoader(DatasetFromFolder(self.args.INPUTS_PATH), batch_size=self.args.train_batch_size, shuffle=True, num_workers=4)
            return(train_loader)
        if data_split == "test":
            test_loader = DataLoader(DatasetFromFolder(self.args.VALID_PATH), batch_size=self.args.val_batch_size, shuffle=False, num_workers=4)
            return(test_loader)
