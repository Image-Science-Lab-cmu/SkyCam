    ...
    def train(self):
        ...
        while opt_steps < self.args.max_opt_steps or cur_epoch < self.args.max_epoch:
            ...
            for batch_x in tqdm(
                ...
                #SkyCam
                timeseries = Variable(batch_x[1]).float().to(self.device).unsqueeze(1) # input GHI associated to keogram
                forecast = Variable(batch_x[2]).float().to(self.device).squeeze(1) # ground truth GHI
                padding_needed = 512 - timeseries.size(2)
                timeseries = torch.nn.functional.pad(timeseries, (padding_needed, 0))
